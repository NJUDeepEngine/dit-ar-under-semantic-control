...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def full_ksu_training_losses(self, model, x_start, t, custom_detailed_log, vae, model_kwargs=None, ss_settings=None):
        # 将缩减时间步映射回原始时间步
        assert t is not None and isinstance(t, th.Tensor)
        mapper = lambda t : np.linspace(0, 999, t + 1).round().astype(int).tolist()
        wrapped_t = timesteps_padding(t=t, mapper=mapper)
        ...
        # 调用父类 training_losses，传入映射后的时间步
        return super().full_ksu_training_losses(model, x_start, wrapped_t['patched_timesteps'], custom_detailed_log, vae, model_kwargs=model_kwargs,ss_settings=ss_settings)
